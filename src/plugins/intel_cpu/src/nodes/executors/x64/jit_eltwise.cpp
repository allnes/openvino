...
InferenceEngine::Precision eltwise_precision_helper::get_precision(Algorithm algorithm,
    ...
    for (size_t i = 1; i < eltwise_post_ops.size(); ++i) {
        std::set<std::vector<element::Type>> prcs = get_supported_precisions(eltwise_post_ops[i].eltwise.algorithm);
...
bool JitEltwiseExecutor::init(const EltwiseAttrs& eltwiseAttrs,
    ...
    // TODO: extend executor API to properly check dynamic shapes case
    canUseOptimizedShapeAgnosticImpl = true; // isDynamicNode();
    for (int i = 0; i < postOps.size(); i++) {
        if (postOps[i].type == EltwisePostOpType::Dnnl) {
    ...
    // TODO: enable along with caching support
    ...
    //         for (int i = 0; i < inputNum; i++) {
    //             if (broadcastPolicy[i] != (inpDims[i].back() == 1)) {
    //                 broadcastPolicy[i] = (inpDims[i].back() == 1);
    ...
    if (canUseOptimizedShapeAgnosticImpl) {
        ...
        outDims.resize(inpDims[0].size(), 1);
        ...
        auto offset_in_calc = [](VectorDims& offset, const VectorDims& inpDims, const VectorDims& outDims) {
            int k = 1;
            for (int i = offset.size() - 1; i >= 0; i--) {
                offset[i] = (inpDims[i] == outDims[i]) ? k : 0;
                k *= inpDims[i];
            }
        };

        auto inputSize = inpDims.front().size();
        outOffsets.resize(inputSize, 1);
        offset_out_calc(outOffsets, outDims);
        for (int j = 0; j < inputSize; j++) {
            outOffsets[j] *= dstDescs[0]->getPrecision().size();
        }

        auto inputsNumber = inpDims.size();
        inOffsets.resize(inputsNumber);
        for (int i = 0; i < inputsNumber; i++) {
            inOffsets[i].resize(inputSize, 1);
            offset_in_calc(inOffsets[i], inpDims[i], outDims);
            for (int j = 0; j < inputSize; j++) {
                inOffsets[i][j] *= srcDescs[i]->getPrecision().size();
            }
        }
    }
    ...
    jep.use_runtime_ptrs = canUseOptimizedShapeAgnosticImpl;
    ...
    while (!canUseOptimizedShapeAgnosticImpl && currentJitWorkAmount < minimalJitWorkAmount && currentJitWorkAmount < fullWorkAmount &&
...
void JitEltwiseExecutor::exec(const std::vector<MemoryCPtr>& src, const std::vector<MemoryPtr>& dst, const void *post_ops_data_) {
    ...
    VectorDims dims_out = canUseOptimizedShapeAgnosticImpl ? execParams.outDims : jep.dims;
