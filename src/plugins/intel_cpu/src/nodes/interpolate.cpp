...
void Interpolate::initSupportedPrimitiveDescriptors() {
    ...
    // For x86, only support i8, u8, bf16
    ...
    auto pushDesc = [&](LayoutType dataFormat,
        ...
        if (useAclExecutor) {
            ...
            // TODO: InterpolateExecutorFactory doesn't exist in the new pattern
    ...
    if (is_version11) {
        ...
        // TODO: Move ACL-specific logic to executor implementations
...
void Interpolate::execute([[maybe_unused]] const dnnl::stream& strm) {
    ...
    if (executorPtr) {
        // Call new executor pattern with MemoryArgs
        MemoryArgs memArgs;
        memArgs[ARG_SRC] = srcMemPtr;
        memArgs[ARG_DST] = dstMemPtr;
        executorPtr->execute(memArgs);
    } else {
