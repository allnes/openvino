...
// Node no longer contains inlined JIT/REF executors; execution is delegated to ExecutorFactory backends.
...
private:
    ...
    // No legacy executors below; execution is fully delegated to ExecutorFactory backends

    // Post-ops/fusing are disabled for Interpolate in the new executor architecture.

    static VectorDims getPaddedInputShape(const VectorDims& srcDims,
                                          const std::vector<int>& padBegin,
                                          const std::vector<int>& padEnd);
    std::vector<float> getScales(const VectorDims& srcDim,
    ...
    // New executor factory path (reference-first)
    ov::intel_cpu::ExecutorFactoryPtr<InterpolateAttrs> m_factory;
    ov::intel_cpu::ExecutorPtr m_executor;
    ov::intel_cpu::MemoryArgs m_memory;
};
